...
const { getCachedUser, clearUserCache, setCachedUser } = require('./cache');
...
// ── Cache-backed user lookup — checks cache first, falls back to DB ──
async function getUser(uid, userObj = null) {
  let user = await getCachedUser(uid);
  if (!user) {
...
module.exports = { connectDB, getSetting, setSetting, getUser, getBalance, clearUserCache };
