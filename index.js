...
const { connectDB, getSetting, setSetting, getUser, getBalance, clearUserCache } = require('./db');
...
// ── Fire-and-forget send: use for non-critical broadcasts where we
//    don't need the message_id back and don't want to block the loop ──
function sendMsgFF(chatId, text, opts = {}) {
  bot.sendMessage(chatId, text, opts).catch(() => { /* silently ignore blocked/deleted */ });
}
...
async function showPlayPanel(chatId, uid) {
  ...
  User.findOneAndUpdate({ uid }, { last_active_round: state.currentRound }).catch(() => {});
...
// ── IMPROVEMENT: only notify users active in the last 3 rounds ──
// Avoids pinging everyone who ever pressed Play once and vanished.
async function getActiveUsers(currentRound) {
  const cutoff = Math.max(0, currentRound - 3);
  const docs = await User.find(
    { last_active_round: { $gte: cutoff }, banned: { $ne: true } },
    'uid'
  ).lean();
...
async function runRound() {
  ...
  // ── IMPROVEMENT: notify only players active in the last 3 rounds, not everyone ever ──
  const activeUids = await getActiveUsers(roundNum);
  const toNotify = activeUids.filter(uid => !state.optedOut.has(uid));
  activeUids.forEach(uid => state.activePlayers.add(uid));
  ...
  // ── Timer update loop — har 10s pe update (reduced Telegram API load) ──
  ...
  while (elapsed < ROUND_TIME * 1000) {
    ...
    // ── Batched, non-blocking edits: fire in parallel, don't await each one serially ──
    await Promise.all(allTimerUids.map(async (uid2) => {
      if (state.optedOut.has(uid2)) return;
  ...
  // ── IMPROVEMENT: timer khatam hote hi seedha result — 0s wait ──
  // (purana 7s countdown removed; result seedha aata hai)
  ...
  [...state.activePlayers]
    ...
    .forEach(uid2 => sendMsgFF(uid2, resultText));
